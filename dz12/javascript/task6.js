...
if (confirm('Почати тестування?')) {
    const arrayOfNames = ['Irina', 'Mykola', 'Denys', 'Oleksandr', 'Oleg', 'Dmytro', 'Borys', 'Oksana', 'Petro', 'Olga', 'Viktoriya'];
     ...
     */
    function findIndexByLength(a, targetLength) {
        let start = 0;
        let end = a.length - 1;
        while (start <= end) {
            const mid = Math.floor((start + end) / 2);
            if (a[mid].length === targetLength)
                return mid;
            else if (a[mid].length < targetLength)
                start = mid + 1;
            else
                end = mid - 1;
        }
